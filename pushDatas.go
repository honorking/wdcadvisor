...
func pushData() {
	...
	slaveNodeName := getSlavenodeName()
	...
	for k := 1; k < len(cadvDataForOneContainer); k++ { //Traversal containers and ignore head
		...
		if isServiceCreatedbySwarm(cadvDataForOneContainer[k]) != true {
			continue
		}
		...
		endpoint := getEndPoint(cadvDataForOneContainer[k]) //there is the hosts file path in the inpect of container
		if endpoint == "" {
			LogErr(err, "do not get endpoint")
			continue
		}

		getCpuNum(cadvDataForOneContainer[k]) //we need to give the container CPU ENV

		tag := getTag(endpoint, slaveNodeName) //recode some other message for a container
		...
		if err := pushAlive(`0`, timestamp, tag, containerId, endpoint); err != nil{
			LogErr(err, "pushAlive status in pushData")
		}
	}
}


func isServiceCreatedbySwarm(cadvisorData string) bool {
	return strings.Contains(cadvisorData, "swarm")
}
...
func pushAlive(value, timestamp, tags, containerId, endpoint string) error {
	if err := pushIt(value, timestamp, `"container.alive"`, tags, containerId, "GAUGE", endpoint); err != nil {
