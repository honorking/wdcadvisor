...
func pushIt(value, timestamp, metric, tags, containerId, counterType, endpoint string) error {
	...
	LogRun(postThing)
	//push data to falcon-transfer
	// because container cannot connect to slavenode' port in overlay network
	url := "http://falcon.internal.wandoujia.com/transfer/api/push"
