...
func getCpuNum(dockerdata string) {
	cpuNum = 1
	quota := getBetween(dockerdata, `"quota":`, `,"period"`)
	if quota != "" {
		fenzi, _ := strconv.ParseInt(quota, 10, 32)
		period := getBetween(dockerdata, `"period":`, `},"has_memory"`)
		fenmu, _ := strconv.ParseInt(period, 10, 32)
		cpuNum = fenzi/fenmu
	}
}

func getTag(cname, nodename string) string {
	//FIXMI:some other message for container
	var tags string
	serviceName := strings.Split(cname, `.`)[0]
	tags = fmt.Sprintf("docker_service=%s,docker_host=%s,type=container",serviceName,nodename)
	return tags
}
...
func getBetween(str, start, end string) string {
	...
	//LogErr(errors.New("DEBUG"),strings.Join(res, "-"))
	if len(res) <= 1 {
...
func getSlavenodeName() (string) {

	filepath := "/etc/host/hostname"
...
func getEndPoint(DockerData string) string {
	...
	endPoint := getBetween(DockerData, `"com.docker.swarm.task.name":"`, `"}`)
	if endPoint != "" {
		return endPoint
	}
